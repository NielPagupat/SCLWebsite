    ...
    <div className='sticky top-0 z-10 flex  justify-between p-3 drop-shadow-xl bg-gradient-to-l from-SCLBlue via-lightgreen to-SCLYellow font-tomorrow rounded-b-2xl'>
      ...
      <div className='hidden md:flex flex-1 justify-end items-center'>
        <div className='relative' ref={dropdownRef} onMouseEnter={() => setIsDropdownOpen(true)}>
          <button className='mr-10 p-3 flex items-center gap-2 text-white font-semibold hover:text-SCLLightBlue hover:scale-110 transition-transform duration-150 ease-in-out active:scale-95'>
            About Us {isDropdownOpen ? <ChevronUp className="w-4 h-4 transition-transform duration-300" /> : <ChevronDown className="w-4 h-4 transition-transform duration-300" />}
          </button>
          {isDropdownOpen && (
            <div className='absolute left-0 mt-2 w-48 bg-white border rounded-lg shadow-lg transition-opacity duration-300' onMouseLeave={() => setIsDropdownOpen(false)}>
              <ul className='py-2 text-Font'>
                <li className='px-4 py-2 hover:rounded-xl hover:bg-gray-100 cursor-pointer hover:scale-105 transition-transform duration-150 ease-in-out active:scale-95' 
                    onClick={goToAboutUs}>
                      Our Story
                </li>
                <li className='px-4 py-2 hover:rounded-xl hover:bg-gray-100 cursor-pointer hover:scale-105 transition-transform duration-150 ease-in-out active:scale-95'
        ...
        <button className='mr-10 p-3 text-white font-semibold hover:text-SCLLightBlue hover:scale-110 transition-transform duration-150 ease-in-out active:scale-95' 
      ...
      {/* Mobile Hamburger Icon */}
        ...
        <div className={`absolute top-16 right-0 rounded-md bg-white mr-6 shadow-lg p-4 flex flex-col space-y-4 md:hidden transition-transform duration-300 ${isMobileMenuOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
          <button className='text-Font font-semibold hover:text-SCLLightBlue hover:scale-110 transition-transform duration-150 ease-in-out active:scale-95' onClick={goToAboutUs}>About Us</button>
          <button className='text-Font font-semibold hover:text-SCLLightBlue hover:scale-110 transition-transform duration-150 ease-in-out active:scale-95' onClick={goToService}>Services</button>
          <button className='text-Font font-semibold hover:text-SCLLightBlue hover:scale-110 transition-transform duration-150 ease-in-out active:scale-95' onClick={goToProductsPortfolio}>Product Portfolio</button>
          <button className='text-Font font-semibold hover:text-SCLLightBlue hover:scale-110 transition-transform duration-150 ease-in-out active:scale-95' onClick={goToExperience}>Experiences</button>
        </div>
      </div>
  );
}
